/** @format */

import React, { Component } from 'react';
import { Link } from 'react-router-dom';
export default class Body extends Component {
	render() {
		return (
			<div className='px-[75px]'>
				<div className='sp'>
					<div className='mt-[15px] min-h-screen bg-white '>
						<div className=' flex justify-between items-center m-2'>
							<h3 className='no-underline font-bold ... hover:underline ... '>
								ĐIỆN THOẠI NỔI BẬT NHẤT
							</h3>
							<div className='flex-1 text-right m-[10px] '>
								<ul>
									<li className='m-1 p-2 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to=''>Reno 7 Z</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Apple</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Samsung</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Xiaomi</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>OPPO</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Realme</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Nokia</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Asus</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Vivo</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Xem tất cả</Link>
									</li>
								</ul>
							</div>
						</div>
						<div className='flex flex-row flex-wrap justify-around'>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
									className=''
								/>
								<h4 className='font-bold'>Iphone SE 2012</h4>
								<p className='text-red-500 font-bold'>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone_13-_pro-5_4_1.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone_13-_pro-5_4.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='	https://image.cellphones.com.vn/220x/media/catalog/product/i/p/ip13-pro_2.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='	https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone_11_white_4_.png'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/y/e/yellow_final_2.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
									className=''
								/>

								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>

								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>

								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>

								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
						</div>
					</div>
					<div className='mt-15px min-h-screen bg-white '>
						<div className=' flex justify-between items-center m-2'>
							<h3 className='no-underline font-bold ... hover:underline ... '>
								LAPTOP
							</h3>
							<div className='flex-1 text-right m-10px '>
								<ul>
									<li className='m-1 p-2 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Thinkpad</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Mac Studio</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Studio Display</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Mac</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>ASUS</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>HP</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Mycrosoft</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Acer</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Dell</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>MSI</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Xem tất cả</Link>
									</li>
								</ul>
							</div>
						</div>
						<div className='flex flex-row flex-wrap justify-around'>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/m/a/mac_air.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='	https://image.cellphones.com.vn/220x/media/catalog/product/m/b/mbp-silver-select-202011-spa.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/2/_/2_64_37.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='	https://image.cellphones.com.vn/220x/media/catalog/product/l/a/laptop-huawei-matebook-14-3.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='	https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone_11_white_4_.png'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/y/e/yellow_final_2.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
									className=''
								/>

								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>

								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>

								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>

								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
						</div>
					</div>
					<div className='mt-15px min-h-screen bg-white '>
						<div className=' flex justify-between items-center m-2'>
							<h3 className='no-underline font-bold ... hover:underline ... '>
								MÁY TÍNH ĐỂ BÀN
							</h3>
							<div className='flex-1 text-right m-10px '>
								<ul>
									<li className='m-1 p-2 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Thinkpad</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Mac Studio</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Studio Display</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Mac</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>ASUS</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>HP</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Mycrosoft</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Acer</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Dell</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>MSI</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Xem tất cả</Link>
									</li>
								</ul>
							</div>
						</div>
						<div className='flex flex-row flex-wrap justify-around'>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone_13-_pro-5_4_1.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone_13-_pro-5_4.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='	https://image.cellphones.com.vn/220x/media/catalog/product/i/p/ip13-pro_2.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='	https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone_11_white_4_.png'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/y/e/yellow_final_2.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>
								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
									className=''
								/>

								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>

								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>

								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
							<div className='m-2  mt-[20px] p-3 rounded-lg duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
								<p className=' bg-gradient-to-r from-cyan-500 to-blue-500 h-[25px] w-[100px] pl-1 rounded-r-lg'>
									Giảm 13%
								</p>

								<img
									alt=''
									src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
									className=''
								/>
								<h4>Tên</h4>
								<p>7.990.000</p>
							</div>
						</div>
					</div>
				</div>
				<h3 className='m-2 no-underline font-bold ... hover:underline ... '>
					PHỤ KIỆN
				</h3>
				<div className='text-center'>
					<div className=' m-2 flex flex-row flex-wrap justify-around'>
						<div className='bg-fuchsia-300 w-[120px]  h-[150px] h-150 rounded-lg  cursor-pointer p-2 m-2'>
							<p className='text-white font-bold'>Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className=''></img>
						</div>
						<div className='bg-fuchsia-300 w-[120px]  h-[150px] h-150 rounded-lg  cursor-pointer p-2 m-2'>
							<p className='text-white font-bold'>Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-43.svg'
								className=''></img>
						</div>{' '}
						<div className='bg-fuchsia-300 w-[120px]  h-[150px] h-150 rounded-lg  cursor-pointer p-2 m-2'>
							<p className='text-white font-bold'>Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-286.svg'
								className=''></img>
						</div>{' '}
						<div className='bg-fuchsia-300 w-[120px]  h-[150px] h-150 rounded-lg  cursor-pointer p-2 m-2'>
							<p className='text-white font-bold'>Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-108.svg'
								className=''></img>
						</div>{' '}
						<div className='bg-fuchsia-300 w-[120px]  h-[150px] h-150 rounded-lg  cursor-pointer p-2 m-2'>
							<p className='text-white font-bold'>Độc quyền</p>
							<img
								alt=''
								src='	https://cellphones.com.vn/media/icons/category/cate-114.svg'
								className=''></img>
						</div>{' '}
						<div className='bg-fuchsia-300 w-[120px]  h-[150px] h-150 rounded-lg  cursor-pointer p-2 m-2'>
							<p className='text-white font-bold'>Độc quyền</p>
							<img
								alt=''
								src='	https://cellphones.com.vn/media/icons/category/cate-122.svg'
								className=''></img>
						</div>{' '}
						<div className='bg-fuchsia-300 w-[120px]  h-[150px] h-150 rounded-lg  cursor-pointer p-2 m-2'>
							<p className='text-white font-bold'>Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-676.svg'
								className=''></img>
						</div>{' '}
						<div className='bg-fuchsia-300 w-[120px]  h-[150px] h-150 rounded-lg  cursor-pointer p-2 m-2'>
							<p className='text-white font-bold'>Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className=''></img>
						</div>{' '}
						<div className='bg-fuchsia-300 w-[120px]  h-[150px] h-150 rounded-lg  cursor-pointer p-2 m-2'>
							<p className='text-white font-bold'>Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className=''></img>
						</div>{' '}
						<div className='bg-fuchsia-300 w-[120px]  h-[150px] h-150 rounded-lg  cursor-pointer p-2 m-2'>
							<p className='text-white font-bold'>Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className=''></img>
						</div>{' '}
						<div className='bg-fuchsia-300 w-[120px]  h-[150px] h-150 rounded-lg  cursor-pointer p-2 m-2'>
							<p className='text-white font-bold'>Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className=''></img>
						</div>{' '}
						<div className='bg-fuchsia-300 w-[120px]  h-[150px] h-150 rounded-lg  cursor-pointer p-2 m-2'>
							<p className='text-white font-bold'>Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className=''></img>
						</div>{' '}
						<div className='bg-fuchsia-300 w-[120px]  h-[150px] h-150 rounded-lg  cursor-pointer p-2 m-2'>
							<p className='text-white font-bold'>Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className=''></img>
						</div>{' '}
						<div className='bg-fuchsia-300 w-[120px]  h-[150px] h-150 rounded-lg  cursor-pointer p-2 m-2'>
							<p className='text-white font-bold'>Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className=''></img>
						</div>{' '}
						<div className='bg-fuchsia-300 w-[120px]  h-[150px] h-150 rounded-lg  cursor-pointer p-2 m-2'>
							<p className='text-white font-bold'>Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className=''></img>
						</div>{' '}
						<div className='bg-fuchsia-300 w-[120px]  h-[150px] h-150 rounded-lg  cursor-pointer p-2 m-2'>
							<p className='text-white font-bold'>Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className=''></img>
						</div>{' '}
						<div className='bg-fuchsia-300 w-[120px]  h-[150px] h-150 rounded-lg  cursor-pointer p-2 m-2'>
							<p className='text-white font-bold'>Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className=''></img>
						</div>{' '}
						<div className='bg-fuchsia-300 w-[120px]  h-[150px] h-150 rounded-lg  cursor-pointer p-2 m-2'>
							<p className='text-white font-bold'>Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className=''></img>
						</div>{' '}
						<div className='bg-fuchsia-300 w-[120px]  h-[150px] h-150 rounded-lg  cursor-pointer p-2 m-2'>
							<p className='text-white font-bold'>Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className=''></img>
						</div>{' '}
						<div className='bg-fuchsia-300 w-[120px]  h-[150px] h-150 rounded-lg  cursor-pointer p-2 m-2'>
							<p className='text-white font-bold'>Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className=''></img>
						</div>
					</div>
					<h3 className='m-2 no-underline text-left font-bold ... hover:underline ... '>
						LINH KIỆN MÁY TÍNH
					</h3>
					<div className='flex flex-row w-4/5 ml-[20px]'>
						<div className='  bg-[#FCA5A5] w-[100px] rounded-lg mr-[15px] '>
							<p className='text-white font-bold'>RAM</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-923.svg'
								className='rounded-lg '></img>
						</div>
						<div className='  bg-[#FDA4AF] w-[100px] rounded-lg mr-[15px] '>
							<p className='text-white font-bold'>RAM</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-924.svg'
								className='rounded-lg '></img>
						</div>
						<div className=' bg-[#F9A8D4] w-[100px] rounded-lg mr-[15px] '>
							<p className='text-white font-bold'>RAM</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-952.svg'
								className='rounded-lg '></img>
						</div>
						<div className='  bg-[#C4B5FD] w-[100px] rounded-lg mr-[15px] '>
							<p className='text-white font-bold'>RAM</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1007.svg'
								className='rounded-lg '></img>
						</div>
						<div className='  bg-[#A5B4FC] w-[100px] rounded-lg mr-[15px] '>
							<p className='text-white font-bold'>RAM</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1008.svg'
								className='rounded-lg '></img>
						</div>
						<div className='  bg-[#93C5FD] w-[100px] rounded-lg mr-[15px] '>
							<p className='text-white font-bold'>RAM</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1009.svg'
								className='rounded-lg '></img>
						</div>
						<div className='  bg-[#6EE7B7] w-[100px] rounded-lg mr-[15px] '>
							<p className='text-white font-bold'>RAM</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1010.svg'
								className='rounded-lg '></img>
						</div>
						<div className='  bg-[#FCD34D] w-[100px] rounded-lg mr-[15px] '>
							<p className='text-white font-bold'>RAM</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1011.svg'
								className='rounded-lg '></img>
						</div>
					</div>
					<h3 className='m-2 no-underline text-left font-bold ... hover:underline ... '>
						CHUYÊN GIA THƯƠNG HIỆU
					</h3>
					<div className='flex flex-row'>
						<div className='w-1/4 m-2  '>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/banner/banner-sis-apple-homepage.png'
								className='rounded-lg'></img>
						</div>
						<div className='w-1/4 m-2  '>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/banner/banner-sis-samsung-homepage.png'
								className='rounded-lg'></img>
						</div>
						<div className='w-1/4 m-2  '>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/banner/banner-sis-asus-homepage.png'
								className='rounded-lg'></img>
						</div>
						<div className='w-1/4 m-2  '>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/banner/banner-sis-xiaomi-homepage.png'
								className='rounded-lg'></img>
						</div>
					</div>
				</div>
			</div>
		);
	}
}
