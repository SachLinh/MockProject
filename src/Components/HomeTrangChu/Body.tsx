/** @format */

import React, { Component } from 'react';
import { Link } from 'react-router-dom';
export default class Body extends Component {
	render() {
		return (
			<div className=''>
				<div className='sp'>
				<div className='flex justify-between items-center m-2'>
					    <h3 className='m-2 no-underline text-left font-bold ... hover:underline ... ' >
						ĐIỆN THOẠI NỔI BẬT NHẤT
					    </h3>
						<div className='flex-1 text-right m-10px '>
								<ul>
									<li className='m-1 p-2 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Thinkpad</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Mac Studio</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Studio Display</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Mac</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>ASUS</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>HP</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Mycrosoft</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Acer</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Dell</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>MSI</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Xem tất cả</Link>
									</li>
								</ul>
							</div>
					</div>
					<div className='dt'>
					<div className=' m-2 flex flex-row flex-wrap justify-around'>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
					</div>
					<div className=' m-2 flex flex-row flex-wrap justify-around'>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone_13-_pro-5_4_1.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src=''
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
					</div>
				</div>
				<div className='flex justify-between items-center m-2'>
					    <h3 className='m-2 no-underline text-left font-bold ... hover:underline ... ' >
						ĐIỆN THOẠI NỔI BẬT NHẤT
					    </h3>
						<div className='flex-1 text-right m-10px '>
								<ul>
									<li className='m-1 p-2 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Thinkpad</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Mac Studio</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Studio Display</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Mac</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>ASUS</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>HP</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Mycrosoft</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Acer</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Dell</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>MSI</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Xem tất cả</Link>
									</li>
								</ul>
							</div>
					</div>
					<div className='dt'>
					<div className=' m-2 flex flex-row flex-wrap justify-around'>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
					</div>
					<div className=' m-2 flex flex-row flex-wrap justify-around'>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
					</div>
				</div>
				<div className='flex justify-between items-center m-2'>
					    <h3 className='m-2 no-underline text-left font-bold ... hover:underline ... ' >
						ĐIỆN THOẠI NỔI BẬT NHẤT
					    </h3>
						<div className='flex-1 text-right m-10px '>
								<ul>
									<li className='m-1 p-2 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Thinkpad</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Mac Studio</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Studio Display</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Mac</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>ASUS</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>HP</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Mycrosoft</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Acer</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Dell</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>MSI</Link>
									</li>
									<li className='p-2 m-1 list-none no-underline inline-block bg-slate-100 rounded-lg'>
										<Link to='#'>Xem tất cả</Link>
									</li>
								</ul>
							</div>
					</div>
					<div className='dt'>
					<div className=' m-2 flex flex-row flex-wrap justify-around'>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
					</div>
					<div className=' m-2 flex flex-row flex-wrap justify-around'>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
						<div className='text-left bg-white b w-[237px]  h-[361px] h-150 rounded-lg re cursor-pointer p-2 m-2 duration-500 shadow-[0_0px_4px_4px_#b6b5b5] hover:shadow-[0_0px_8px_8px_#868585]'>
							<p className='bg-gradient-to-r from-cyan-500 to-blue-500 h-[30px] w-[100px] pl-1 rounded-r-lg leading-[30px] '>Giảm <span>13%</span></p>
							<img
								alt=''
								src='https://image.cellphones.com.vn/220x/media/catalog/product/i/p/iphone-se-red-select-20220322.jpg'
								className='w-[160px] h-[160px] text-center m-auto mt-[15px]'></img>
								<h4 className='font-bold'>Iphone SE 2022| Chính hãng VN/A</h4>
								<p className='text-red-600 font-bold  m-1'>12.990.000</p>
						</div>
					</div>
				</div>
				</div>
				
				<div className='m-[15px] '>
					<div className='pk'>
					<h3 className='m-2 no-underline font-bold ... hover:underline ... '>
					PHỤ KIỆN
				    </h3>
				   <div className='flex flex-row'>
				   <div className='w-1/10 mr-[15px] text-left bg-indigo-200 b w-[111px]  h-[127px] rounded-lg re cursor-pointer '>
							<p className='absolute p-[14px] font-bold text-white'> Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className='w-[111px] h-[127px]'></img>
						</div>
						<div className='w-1/10 mr-[15px] text-left bg-indigo-200 b w-[111px]  h-[127px] rounded-lg re cursor-pointer '>
							<p className='absolute p-[14px] font-bold text-white'> Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className='w-[111px] h-[127px]'></img>
						</div>
						<div className='w-1/10 mr-[15px] text-left bg-indigo-200 b w-[111px]  h-[127px] rounded-lg re cursor-pointer '>
							<p className='absolute p-[14px] font-bold text-white'> Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className='w-[111px] h-[127px]'></img>
						</div>
						<div className='w-1/10 mr-[15px] text-left bg-indigo-200 b w-[111px]  h-[127px] rounded-lg re cursor-pointer '>
							<p className='absolute p-[14px] font-bold text-white'> Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className='w-[111px] h-[127px]'></img>
						</div>
						<div className='w-1/10 mr-[15px] text-left bg-indigo-200 b w-[111px]  h-[127px] rounded-lg re cursor-pointer '>
							<p className='absolute p-[14px] font-bold text-white'> Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className='w-[111px] h-[127px]'></img>
						</div>
						<div className='w-1/10 mr-[15px] text-left bg-indigo-200 b w-[111px]  h-[127px] rounded-lg re cursor-pointer '>
							<p className='absolute p-[14px] font-bold text-white'> Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className='w-[111px] h-[127px]'></img>
						</div>
						<div className='w-1/10 mr-[15px] text-left bg-indigo-200 b w-[111px]  h-[127px] rounded-lg re cursor-pointer '>
							<p className='absolute p-[14px] font-bold text-white'> Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className='w-[111px] h-[127px]'></img>
						</div>
						<div className='w-1/10 mr-[15px] text-left bg-indigo-200 b w-[111px]  h-[127px] rounded-lg re cursor-pointer '>
							<p className='absolute p-[14px] font-bold text-white'> Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className='w-[111px] h-[127px]'></img>
						</div>
						<div className='w-1/10 mr-[15px] text-left bg-indigo-200 b w-[111px]  h-[127px] rounded-lg re cursor-pointer '>
							<p className='absolute p-[14px] font-bold text-white'> Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className='w-[111px] h-[127px]'></img>
						</div>
						<div className='w-1/10 mr-[15px] text-left bg-indigo-200 b w-[111px]  h-[127px] rounded-lg re cursor-pointer '>
							<p className='absolute p-[14px] font-bold text-white'> Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className='w-[111px] h-[127px]'></img>
						</div>
						
					</div>
					<div className='flex flex-row mt-[10px]'>
				   <div className='w-1/10 mr-[15px] text-left bg-indigo-200 b w-[111px]  h-[127px] rounded-lg re cursor-pointer '>
							<p className='absolute p-[14px] font-bold text-white'> Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className='w-[111px] h-[127px]'></img>
						</div>
						<div className='w-1/10 mr-[15px] text-left bg-indigo-200 b w-[111px]  h-[127px] rounded-lg re cursor-pointer '>
							<p className='absolute p-[14px] font-bold text-white'> Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className='w-[111px] h-[127px]'></img>
						</div>
						<div className='w-1/10 mr-[15px] text-left bg-indigo-200 b w-[111px]  h-[127px] rounded-lg re cursor-pointer '>
							<p className='absolute p-[14px] font-bold text-white'> Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className='w-[111px] h-[127px]'></img>
						</div>
						<div className='w-1/10 mr-[15px] text-left bg-indigo-200 b w-[111px]  h-[127px] rounded-lg re cursor-pointer '>
							<p className='absolute p-[14px] font-bold text-white'> Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className='w-[111px] h-[127px]'></img>
						</div>
						<div className='w-1/10 mr-[15px] text-left bg-indigo-200 b w-[111px]  h-[127px] rounded-lg re cursor-pointer '>
							<p className='absolute p-[14px] font-bold text-white'> Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className='w-[111px] h-[127px]'></img>
						</div>
						<div className='w-1/10 mr-[15px] text-left bg-indigo-200 b w-[111px]  h-[127px] rounded-lg re cursor-pointer '>
							<p className='absolute p-[14px] font-bold text-white'> Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className='w-[111px] h-[127px]'></img>
						</div>
						<div className='w-1/10 mr-[15px] text-left bg-indigo-200 b w-[111px]  h-[127px] rounded-lg re cursor-pointer '>
							<p className='absolute p-[14px] font-bold text-white'> Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className='w-[111px] h-[127px]'></img>
						</div>
						<div className='w-1/10 mr-[15px] text-left bg-indigo-200 b w-[111px]  h-[127px] rounded-lg re cursor-pointer '>
							<p className='absolute p-[14px] font-bold text-white'> Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className='w-[111px] h-[127px]'></img>
						</div>
						<div className='w-1/10 mr-[15px] text-left bg-indigo-200 b w-[111px]  h-[127px] rounded-lg re cursor-pointer '>
							<p className='absolute p-[14px] font-bold text-white'> Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className='w-[111px] h-[127px]'></img>
						</div>
						<div className='w-1/10 mr-[15px] text-left bg-indigo-200 b w-[111px]  h-[127px] rounded-lg re cursor-pointer '>
							<p className='absolute p-[14px] font-bold text-white'> Độc quyền</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-1644.svg'
								className='w-[111px] h-[127px]'></img>
						</div>
						
					</div>
				   </div>
					<h3 className='m-2 no-underline text-left font-bold ... hover:underline ... '>
						LINH KIỆN MÁY TÍNH
					</h3>
					<div className='flex flex-row w-4/5'>
						<div className='w-[111px] h-[127px]  bg-[#FCA5A5] rounded-lg relative m-[10px] '>
							<p className='text-white font-bold absolute'>RAM</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-923.svg'
								className='rounded-lg w-[111px] h-[127px]'></img>
						</div>
						<div className='w-[111px] h-[127px]  bg-[#FCA5A5] rounded-lg relative m-[10px] '>
							<p className='text-white font-bold absolute'>RAM</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-923.svg'
								className='rounded-lg w-[111px] h-[127px]'></img>
						</div>
						<div className='w-[111px] h-[127px]  bg-[#FCA5A5] rounded-lg relative m-[10px] '>
							<p className='text-white font-bold absolute'>RAM</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-923.svg'
								className='rounded-lg w-[111px] h-[127px]'></img>
						</div>
						<div className='w-[111px] h-[127px]  bg-[#FCA5A5] rounded-lg relative m-[10px] '>
							<p className='text-white font-bold absolute'>RAM</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-923.svg'
								className='rounded-lg w-[111px] h-[127px]'></img>
						</div>
						<div className='w-[111px] h-[127px]  bg-[#FCA5A5] rounded-lg relative m-[10px] '>
							<p className='text-white font-bold absolute'>RAM</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-923.svg'
								className='rounded-lg w-[111px] h-[127px]'></img>
						</div>
						<div className='w-[111px] h-[127px]  bg-[#FCA5A5] rounded-lg relative m-[10px] '>
							<p className='text-white font-bold absolute'>RAM</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-923.svg'
								className='rounded-lg w-[111px] h-[127px]'></img>
						</div>
						<div className='w-[111px] h-[127px]  bg-[#FCA5A5] rounded-lg relative m-[10px] '>
							<p className='text-white font-bold absolute'>RAM</p>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/category/cate-923.svg'
								className='rounded-lg w-[111px] h-[127px]'></img>
						</div>
						
					</div>
					<h3 className='m-2 no-underline text-left font-bold ... hover:underline ... '>
						CHUYÊN GIA THƯƠNG HIỆU
					</h3>
					<div className='flex flex-row'>
						<div className='w-1/4 m-2  '>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/banner/banner-sis-apple-homepage.png'
								className='rounded-lg'></img>
						</div>
						<div className='w-1/4 m-2  '>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/banner/banner-sis-samsung-homepage.png'
								className='rounded-lg'></img>
						</div>
						<div className='w-1/4 m-2  '>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/banner/banner-sis-asus-homepage.png'
								className='rounded-lg'></img>
						</div>
						<div className='w-1/4 m-2  '>
							<img
								alt=''
								src='https://cellphones.com.vn/media/icons/banner/banner-sis-xiaomi-homepage.png'
								className='rounded-lg'></img>
						</div>
					</div>
				</div>
			</div>
		);
	}
}
